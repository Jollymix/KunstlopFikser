...
class ModernButton(tk.Canvas):
    ...
    def __init__(self, parent, text, command=None, width=120, height=36, 
                 bg_color="#2563eb", hover_color="#1d4ed8", text_color="white",
                 disabled_color="#9ca3af", **kwargs):
...
class App:
    def __init__(self, root):
        ...
        # Source section
        self.create_source_section(left_col)
        
        # Log section
        self.create_log_section(left_col)
        ...
        # Reports section
        self.create_reports_section(right_col)
        
        # Startliste section
        self.create_startliste_section(right_col)
    ...
    def create_header(self, parent):
        ...
        # Icon/Logo area (you can add an icon here)
        icon_frame = ttk.Frame(header, style="Main.TFrame")
        icon_frame.pack(side="left", padx=(0, 15))
        
        # Create a simple icon using canvas
        icon = tk.Canvas(icon_frame, width=48, height=48, 
                        bg=self.colors['primary'], highlightthickness=0)
        icon.pack()
        icon.create_oval(4, 4, 44, 44, fill=self.colors['primary'], outline="")
        icon.create_text(24, 24, text="FSM", fill="white", 
        ...
        ttk.Label(title_frame, text="FSM Data Dekoder", 
                 style="Title.TLabel").pack(anchor="w")
        ttk.Label(title_frame, 
                 text="Behandle konkurransedata og generer rapporter",
    ...
    def create_source_section(self, parent):
        ...
        # Section title
        ttk.Label(card, text="📁 Datakilde", 
                 style="SectionTitle.TLabel").pack(anchor="w", pady=(0, 12))
        
        # Folder input
        input_frame = ttk.Frame(card, style="Card.TFrame")
        input_frame.pack(fill="x", pady=(0, 12))
        
        ttk.Label(input_frame, text="Mappe:", 
                 style="CardLabel.TLabel").pack(side="left", padx=(0, 8))
        
        entry = ttk.Entry(input_frame, textvariable=self.folder_var, 
                         font=("Segoe UI", 10))
        entry.pack(side="left", fill="x", expand=True, padx=(0, 8))
        
        browse_btn = ModernButton(input_frame, text="Bla gjennom", 
        ...
        # Scan button
        self.scan_btn = ModernButton(card, text="🔍 Skann Filer", 
                                     command=self.read_zip,
                                     width=180, height=40,
                                     bg_color=self.colors['primary'],
                                     hover_color=self.colors['primary_hover'])
        self.scan_btn.pack(pady=(0, 0))
    ...
    def create_log_section(self, parent):
        ...
        ttk.Label(title_frame, text="📋 Aktivitetslogg", 
                 style="SectionTitle.TLabel").pack(side="left")
        
        # Log widget with custom styling
        log_frame = tk.Frame(card, bg=self.colors['card'])
        log_frame.pack(fill="both", expand=True)
        
        self.log_widget = ScrolledText(log_frame, 
                                       font=("Consolas", 9),
                                       bg="#1e293b",
                                       fg="#e2e8f0",
    ...
    def create_reports_section(self, parent):
        ...
        self.chk_excel = ttk.Checkbutton(checkbox_frame, text="📊 Excel", 
                                        variable=self.var_excel,
                                        style="Card.TCheckbutton")
        self.chk_excel.pack(anchor="w", pady=4)
        
        self.chk_html = ttk.Checkbutton(checkbox_frame, text="🌐 HTML", 
                                       variable=self.var_html,
                                       style="Card.TCheckbutton")
        self.chk_html.pack(anchor="w", pady=4)
        
        self.chk_pdf = ttk.Checkbutton(checkbox_frame, text="📑 PDF", 
                                      variable=self.var_pdf,
                                      style="Card.TCheckbutton")
        self.chk_pdf.pack(anchor="w", pady=4)
        
        # Generate button
        self.btn_generate = ModernButton(card, text="✨ Generer Filer", 
    ...
    def create_startliste_section(self, parent):
        card = ModernCard(parent, padding=20)
        card.pack(fill="both", expand=True)
        
        ttk.Label(card, text="🏁 Startliste", 
                 style="SectionTitle.TLabel").pack(anchor="w", pady=(0, 12))
        
        # Date and time inputs
        dt_frame = ttk.Frame(card, style="Card.TFrame")
        dt_frame.pack(fill="x", pady=(0, 8))
        
        today_str = datetime.now().strftime("%d.%m.%y")
        self.start_date_var = tk.StringVar(value=today_str)
        self.start_time_var = tk.StringVar(value="18:00")
        
        ttk.Label(dt_frame, text="Dato:", style="CardLabel.TLabel").grid(
            row=0, column=0, sticky="w", pady=4)
        ttk.Entry(dt_frame, textvariable=self.start_date_var, 
                 width=12, font=("Segoe UI", 9)).grid(
            row=0, column=1, sticky="ew", padx=(8, 0), pady=4)
        
        ttk.Label(dt_frame, text="Kl:", style="CardLabel.TLabel").grid(
            row=1, column=0, sticky="w", pady=4)
        ttk.Entry(dt_frame, textvariable=self.start_time_var, 
                 width=12, font=("Segoe UI", 9)).grid(
        ...
        ttk.Label(settings_frame, text="Intervall:", 
                 style="CardLabel.TLabel").grid(row=0, column=0, sticky="w", pady=4)
        ttk.Entry(settings_frame, textvariable=self.interval_var, 
                 width=12, font=("Segoe UI", 9)).grid(
            row=0, column=1, sticky="ew", padx=(8, 0), pady=4)
        
        ttk.Label(settings_frame, text="Gruppe:", 
                 style="CardLabel.TLabel").grid(row=1, column=0, sticky="w", pady=4)
        ttk.Entry(settings_frame, textvariable=self.group_size_var, 
                 width=12, font=("Segoe UI", 9)).grid(
            row=1, column=1, sticky="ew", padx=(8, 0), pady=4)
        
        ttk.Label(settings_frame, text="Sted:", 
                 style="CardLabel.TLabel").grid(row=2, column=0, sticky="w", pady=4)
        ttk.Entry(settings_frame, textvariable=self.location_var, 
                 width=12, font=("Segoe UI", 9)).grid(
        ...
        ttk.Checkbutton(card, text="🔀 Tilfeldig rekkefølge", 
                       variable=self.shuffle_var,
                       style="Card.TCheckbutton").pack(anchor="w", pady=(0, 12))
        
        # Generate button
        self.btn_startliste = ModernButton(card, text="🚀 Lag Startliste", 
                                          command=self.generate_startliste,
                                          width=200, height=38,
                                          bg_color=self.colors['warning'],
                                          hover_color="#d97706")
        self.btn_startliste.pack()
    ...
    def set_output_controls(self, enabled):
        ...
        self.btn_generate.config_state(state)
        self.btn_startliste.config_state(state)
    ...
    def generate_startliste(self):
        ...
        try:
            ...
            messagebox.showerror("Feil", "Ugyldig gruppe-størrelse. Bruk et tall.")
            return
        if group_size <= 0:
            messagebox.showerror("Feil", "Gruppe-størrelse må være > 0.")
            return
